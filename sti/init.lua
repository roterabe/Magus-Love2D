...
-- Custom function to return instance of tile with standard pixel coordinates. Accept x and y coordinates + a layer index if specified.
function Map:getInstanceByPixel(x, y, layer)
    ...
    if tiley < 120 or tilex > 4 then
        if self.layers[layer].data[tiley][tilex + 1].gid then
            local gid = self.layers[layer].data[tiley][tilex + 1].gid
...
-- Custom function to remove tile by instance.
function Map:removeInstance(instance)
    ...
    tilex, tiley = math.floor(tilex), math.floor(tiley)
    -- instance.layer.data[tilex+1][tiley+1] = 0
end
